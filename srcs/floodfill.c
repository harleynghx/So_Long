...
{
    return (row >= 0 && row < list->n_height && col >= 0 && col < list->n_width && map[row][col] != '1');
}
    ...
    while (ff_data->track < ff_data->append)
    {
        ...
        while (ff_data->dir < 4)
        {
            ...
            if (is_valid(list, ff_data->new_y, ff_data->new_x, ff_data->map))
            {
                ...
                queue[ff_data->append++] = (t_point){ ff_data->new_y, ff_data->new_x };
            }
            ff_data->dir++;
        }
        ff_data->dir = 0;
...
{   
    t_point   queue[list->n_width * list->n_height];
    t_floodfill ff_data;

    ff_data = (t_floodfill){.dir_y = {-1, 1, 0, 0}, .dir_x = {0, 0, -1, 1}, .append = 0,
        .track = 0, .dir = 0, .map = list->map
    };
